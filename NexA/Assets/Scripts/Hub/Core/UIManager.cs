...
namespace NexA.Hub.Core
{
    ...
    public class UIManager : MonoBehaviour
    {
        ...
        [SerializeField] private HubTopBar hubTopBar;
        // SocialPanel est géré par HomeScreen directement (enfant de Home Screen)

        // Écrans pour lesquels la top bar est visible
        private static readonly HashSet<ScreenType> TopBarScreens = new()
        {
            ScreenType.Home, ScreenType.Profile, ScreenType.Friends,
            ScreenType.MatchHistory, ScreenType.MatchDetails
        };
        ...
        private static readonly Dictionary<ScreenType, List<ScreenType>> AllowedTransitions = new()
        {
            ...
            { ScreenType.Profile,      new() { ScreenType.Home, ScreenType.Friends, ScreenType.MatchHistory } },
            { ScreenType.Friends,      new() { ScreenType.Home, ScreenType.Profile, ScreenType.MatchHistory } },
            { ScreenType.MatchHistory, new() { ScreenType.Home, ScreenType.Profile, ScreenType.Friends, ScreenType.MatchDetails } },
            { ScreenType.MatchDetails, new() { ScreenType.MatchHistory, ScreenType.Home } }
        };
        ...
        private async Task TransitionToScreen(ScreenType screenType, object data)
        {
            ...
            try
            {
                ...
                bool isHubScreen = TopBarScreens.Contains(screenType);

                // 4. Top bar : afficher/cacher + onglet actif
                if (hubTopBar != null)
                {
                    if (isHubScreen) { hubTopBar.Show(); hubTopBar.SetActiveTab(screenType); }
                    else               hubTopBar.Hide();
                }
                // Note : SocialPanel est géré par HomeScreen.ShowAsync/HideAsync
